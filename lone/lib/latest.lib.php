...
function latest($skin_dir='', $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options='')
{
    ...
    if(!G5_USE_CACHE || $cache_fwrite) {
        ...
        if(in_array($bo_table,array('MENU_01_SUB_01','MENU_01_SUB_02','MENU_01_SUB_03','MENU_01_SUB_04','MENU_01_SUB_05','MENU_01_SUB_06','MENU_01_SUB_07','MENU_01_SUB_08','MENU_01_SUB_09'))){
            $sql = " select * from {$tmp_write_table} as a left join {$g5['board_file_table']} as b on a.wr_id = b.wr_id where bo_table = '{$bo_table}' and bf_type=2 and wr_is_comment = 0 group by a.wr_id order by wr_num limit 0, {$rows} ";
        } // 메인에 최신글을 첨부된 이미지가 있는것만 골라서 최신글에 나오게 한다 이미지가 없는 게시물은 노출이 안됨
